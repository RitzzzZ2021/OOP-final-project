...
void InfoManager::delete_diary(const QDate& date)
{
    ...
    QSqlQuery query_paragraphs(db);
    QSqlQuery query_para_tags(db);
    query_diary.prepare("delete from diary where diary_date = ?");
    query_paragraphs.prepare("delete from paragraph where diary_date = ?");
    query_para_tags.prepare("delete from para_tags where diary_date = ?");
...
bool InfoManager::diary_exist(const QDate& date)
{
    ...
    bool success;
    QSqlQuery query(db);
    query.prepare("select * from diary where diary_date = ?");
...
Diary* InfoManager::query_date(const QString& d)
{
    ...
    bool success;
    QSqlQuery query_diary(db);
    query_diary.prepare("select * from diary where diary_date = ?");
    query_diary.bindValue(0, d);
    success = query_diary.exec();
    if(!success) { throw EXCEP_EXE_FAILED; }
    if(query_diary.size() == 0) { return nullptr;}
    Diary *ret;
    ...
    query_para.prepare("select * from paragraph where diary_date = ? order by paragraph_id");
...
QueryDiary* InfoManager::query_time(const QDate& begin_date, const QDate& end_date)
{
    ...
    bool success;
    QSqlQuery query(db);
    query.prepare("select diary_date from diary where diary_date >= ? and diary_date <= ? order by diary_date");
