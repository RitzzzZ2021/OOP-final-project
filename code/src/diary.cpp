...
{
    // qDebug() << "paragraphs size()" << paragraphs.size() << endl;
    qDebug() << "do add_paragraphs" << endl;
    if(pos < 0 || pos > paragraphs.size()) { qDebug() << "diary_" << endl;throw(EXCEP_DES_POS); }
    qDebug() << "before set diary belong id" << endl;
    paragraph->set_diary_belong_id(diary_date);
    qDebug() << "diary_" << endl;
    paragraphs.insert(pos, paragraph);
    QList<Paragraph *>::iterator iter;
    int cnt = 0;
    qDebug() << "insert done" << endl;
    for(iter = paragraphs.begin(); iter != paragraphs.end(); iter++)
    {
        Paragraph *t = *iter;
        t->set_para_id(cnt);
